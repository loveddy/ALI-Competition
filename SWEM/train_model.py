...
def build_model(voc_size, emb_dim):
...
def train_iter(model, criterion, optimizer):
    ...
    batch_size = 512
    optimizer.zero_grad()
    sens_1, sens_2, label = helper.get_random_batch(batch_size, mode='train')
...
def train_test(model):
    with torch.no_grad():
        ...
        for j in range(batch_size):
            lo = 0
            if output.data[j][0] > output.data[j][1]:
                lo = 0
            else:
                lo = 1
...
helper = Utils()
helper.build_dic()
helper.process_data(os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), '../data/temp_train/sens'),
                    os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), '../data/temp_train/label'))


model = build_model(len(helper.dic), 300)
...
for i in range(10):
    ...
    #         accuracy) + '        ' + str(f1_score) + '       ' + str(round(sum(loss_list[-50:]) / 50.0, 4))
