...
def build_model(voc_size, emb_dim, hidden_size):
    ...
    if os.path.exists('./ckpt/encoder'):
        encoder.load_state_dict(torch.load('./ckpt/encoder'))
    else:
        word2vec = Word2Vec.load('./ckpt/word2vec_init/model')
    ...
    if os.path.exists('./ckpt/classifier'):
        classifier.load_state_dict(torch.load('./ckpt/classifier'))
...
helper = Utils('../data/train_add', '../data/label_add')
...
print "epoch    precision    recall    accuracy    F1_score    loss"
for i in range(20000):
    ...
    if (i + 1) % 100 == 0:
        ...
        torch.save(encoder.state_dict(), './ckpt/encoder')
        torch.save(classifier.state_dict(), './ckpt/classifier')
