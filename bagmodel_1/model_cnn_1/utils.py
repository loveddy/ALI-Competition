...
TRAIN_DIC_PATH = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), '../../data/bag_train/sens_1.json')
TRAIN_LABEL_DIC_PATH = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), '../../data/bag_train/labels_1.json')
    ...
    def get_random_batch(self, batch_size, mode='train'):
        ...
        if mode == 'train':
            ...
            for i in range(int(batch_size / 2)):
                ...
                data.append(tt)
        if mode == 'test':
            for i in range(int(batch_size * 1 / 2)):
                index = np.random.randint(0, len(self.label_dic) - 1)
                while index in sample or self.label_dic[str(index)] == '1':
                    index = np.random.randint(0, len(self.label_dic) - 1)
                sample.append(index)
                ...
                label.append('-1')
                data.append(tt)
            for i in range(batch_size - int(batch_size * 1 / 2)):
                index = np.random.randint(0, len(self.label_dic) - 1)
                while index in sample or self.label_dic[str(index)] == '0':
                    index = np.random.randint(0, len(self.label_dic) - 1)
                sample.append(index)
