...
helper = Utils()
helper.build_dic()
helper.process_data(os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), '../../data/bag_train/sens_1'),
                    os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), '../../data/bag_train/labels_1'))

kernel_size = 3
model = build_model(kernel_size, len(helper.dic), 50)
...
for i in range(10000):
    ...
    if (i + 1) % 1 == 0:
        ...
        print str(i + 1) + '        ' + str(precision) + '       ' + str(recall) + '      ' + str(
            accuracy) + '        ' + str(f1_score) + '       ' + str(round(sum(loss_list[-10:]) / 10.0, 4))
    if (i + 1) % 500 == 0:
